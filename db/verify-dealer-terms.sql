...
BEGIN
 BEGIN
  ...
  IF (SELECT total_amount FROM orders WHERE id=oid) <> 0 OR (SELECT goods_amount FROM orders WHERE id=oid) <> 650 THEN RAISE EXCEPTION 'FAIL: unsold goods counted as sales'; END IF;
  ...
  IF (SELECT total_amount FROM orders WHERE id=oid) <> 195 THEN RAISE EXCEPTION 'FAIL: 3 sold should owe 195'; END IF;
  PERFORM collect_order(oid,1,true);
  IF (SELECT paid_amount FROM orders WHERE id=oid) <> 195 THEN RAISE EXCEPTION 'FAIL: first payment'; END IF;
  BEGIN
  ...
  IF (SELECT total_amount-paid_amount FROM orders WHERE id=oid) <> 130 OR (SELECT payment_status FROM orders WHERE id=oid) <> 'unpaid' THEN RAISE EXCEPTION 'FAIL: partial collections'; END IF;
  ...
  IF (SELECT total_amount FROM orders WHERE id=buyid) <> 280 THEN RAISE EXCEPTION 'FAIL: buyout discount'; END IF;
