ALTER TABLE customers ADD COLUMN IF NOT EXISTS sale_mode text NOT NULL DEFAULT 'buyout' CHECK(sale_mode IN ('buyout','consignment'));
-- statement
ALTER TABLE customers ADD COLUMN IF NOT EXISTS discount numeric(4,2) CHECK(discount > 0 AND discount <= 10);
-- statement
ALTER TABLE orders ADD COLUMN IF NOT EXISTS sale_mode text NOT NULL DEFAULT 'buyout' CHECK(sale_mode IN ('buyout','consignment'));
-- statement
ALTER TABLE orders ADD COLUMN IF NOT EXISTS discount numeric(4,2), ADD COLUMN IF NOT EXISTS goods_amount numeric(12,2), ADD COLUMN IF NOT EXISTS paid_amount numeric(12,2), ADD COLUMN IF NOT EXISTS revision integer NOT NULL DEFAULT 0;
-- statement
ALTER TABLE order_items ADD COLUMN IF NOT EXISTS retail_price numeric(10,2), ADD COLUMN IF NOT EXISTS sold_qty integer NOT NULL DEFAULT 0, ADD COLUMN IF NOT EXISTS returned_qty integer NOT NULL DEFAULT 0, ADD CONSTRAINT consignment_quantities CHECK(sold_qty >= 0 AND returned_qty >= 0 AND sold_qty + returned_qty <= qty);
-- statement
...
BEGIN
 ...
 INSERT INTO orders(id,customer_id,note,sale_mode,discount,total_amount,goods_amount,paid_amount) VALUES(oid,c.id,p_note,c.sale_mode,c.discount,0,0,0);
 FOR i IN SELECT value FROM jsonb_array_elements(p_items) LOOP
  ...
  total:=total+n*price;
 END LOOP;
 UPDATE orders SET goods_amount=total,total_amount=CASE WHEN c.sale_mode='consignment' THEN 0 ELSE total END WHERE id=oid;
 RETURN oid;
END $$;
-- statement
...
BEGIN
 ...
 UPDATE orders SET total_amount=due,revision=revision+1,payment_status=CASE WHEN due > 0 AND coalesce(paid_amount,0)>=due THEN 'paid' ELSE 'unpaid' END WHERE id=p_order;
 INSERT INTO consignment_events(order_id,event_type,details) VALUES(p_order,'settlement',p_items);
END $$;
-- statement
CREATE OR REPLACE FUNCTION collect_order(p_order uuid,p_revision integer,p_paid boolean) RETURNS void LANGUAGE plpgsql SET search_path=public,pg_temp AS $$
DECLARE o orders%ROWTYPE;
BEGIN
 SELECT * INTO o FROM orders WHERE id=p_order FOR UPDATE;
 IF NOT FOUND THEN RAISE EXCEPTION '找不到訂單'; END IF;
 IF p_revision IS DISTINCT FROM o.revision THEN RAISE EXCEPTION '單據已更新，請重新整理後再操作'; END IF;
 IF o.sale_mode='consignment' AND (NOT p_paid OR o.status<>'shipped' OR o.total_amount<=coalesce(o.paid_amount,0)) THEN RAISE EXCEPTION '寄賣僅能收取已售出且尚未收款的金額'; END IF;
 UPDATE orders SET paid_amount=CASE WHEN p_paid THEN total_amount ELSE 0 END,payment_status=CASE WHEN p_paid THEN 'paid' ELSE 'unpaid' END,revision=revision+1 WHERE id=p_order;
 IF o.sale_mode='consignment' THEN INSERT INTO consignment_events(order_id,event_type,details) VALUES(p_order,'payment',jsonb_build_object('amount',o.total_amount-coalesce(o.paid_amount,0))); END IF;
END $$;
-- statement
REVOKE ALL ON FUNCTION create_dealer_order(uuid,text,jsonb,numeric,text), settle_consignment(uuid,integer,jsonb), collect_order(uuid,integer,boolean) FROM PUBLIC;
