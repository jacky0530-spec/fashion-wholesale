...
「進貨單」頁可填日期、供應商、倉庫、單號，明細選既有商品規格，自動帶入貨號／品名／色碼及進價。未稅單價、數量、稅額均可輸入；每列預設稅額為小計 5%，手動修改後保留手動稅額，也可按「重算 5%」。免稅請填 0。供應商與倉庫支援自由輸入及歷史名稱提示。

單號可自填或產生 PO-日期-流水號。儲存和規格加庫存為同一交易；相同請求重試不重複入庫，單號不可重複。作廢保留主檔與明細並扣回數量，庫存不足時整筆拒絕；重複作廢不再扣庫存。已入庫單據修改方式為作廢後重開。倉庫目前為收貨地點紀錄，庫存仍按商品規格彙總，並非各倉庫獨立結存。進貨不會覆寫商品預設進價。

依序執行 db/purchases.sql 建立主檔、明細、過帳與作廢函式（正式庫已套用）。db/verify-purchases.sql 驗證金額、入庫、重複請求、失敗回復及作廢沖回；測試資料自動回復，流水號可能留有跳號。
