...
商品零售價是含稅定價；依商業議價規則直接乘折數作為未稅成交單價，不先除以 1.05。買斷預設 5.5 折、寄賣預設 6 折，客戶切換合作方式自動帶入。未稅小計為成交單價乘數量之和，另加 5% 稅；單價及稅額保留小數兩位（稅额依整單／累計售出小計計算）。例如零售價 100 元，買斷未稅 55 元，稅 2.75 元，含稅 57.75 元；寄賣未稅 60 元，售出後含稅 63 元。

新單保留 tax_rate、net_amount、tax_amount、goods_net_amount；total_amount 為含稅應收，goods_amount 為含稅貨值。寄賣僅售出部分列銷售小計與稅額，分批收款按累計含稅應收扣除已收款，避免重複收稅。舊單 tax_rate=0，金額維持舊制，後續寄賣結算也沿用原單稅率。

在 dealer-terms.sql 後執行 db/dealer-tax.sql（正式庫已套用）。驗證 db/verify-dealer-tax.sql 與更新後的 db/verify-dealer-terms.sql 均會自動回復測試資料。
