...
const newForm = () => ({ purchase_date: today(), supplier: '', warehouse: '總倉', purchase_no: '' })
...
export default function Purchases({ showToast }) {
  ...
  const load = useCallback(async () => {
    ...
    const [p, w] = await Promise.all([
      request('list', { table: 'purchases' }),
      request('list', { table: 'warehouses' }),
    ])
    setLoading(false)
    const error = p.error || w.error
  ...
  const validWarehouse = warehouses.some(w => w.name === form.warehouse)
  const valid = rows.length > 0 && form.purchase_date && form.supplier.trim() && validWarehouse && rows.every(r => Number.isInteger(Number(r.qty)) && Number(r.qty) > 0 && r.unit_price !== '' && Number.isFinite(Number(r.unit_price)) && Number(r.unit_price) >= 0 && r.tax_amount !== '' && Number.isFinite(Number(r.tax_amount)) && Number(r.tax_amount) >= 0)

  const openNew = () => {
    const defaultWarehouse = warehouses.find(w => w.name === '總倉')?.name || warehouses[0]?.name || '總倉'
    setForm({ ...newForm(), warehouse: defaultWarehouse })
    setRows([]); setRequestId(crypto.randomUUID()); setProductId(''); setVariantId(''); setProductSearch(''); setOpen(true)
  }
  const addRow = () => {
  ...
  const save = async () => {
    ...
    setBusy(true)
    const record = { ...form, supplier: form.supplier.trim(), purchase_no: form.purchase_no.trim(), items: rows.map(r => ({ variant_id: r.variant_id, qty: Number(r.qty), unit_price: Number(r.unit_price), tax_amount: Number(r.tax_amount) })) }
    const r = await request('postPurchase', { table: 'purchases', id: requestId, record })
    if (!r.error) { setOpen(false); showToast(`進貨完成，已增加「${form.warehouse}」庫存`); await Promise.all([load(), reloadProducts()]) }
    setBusy(false)
  }
  const voidDocument = async p => {
    if (busy || !confirm(`作廢進貨單 ${p.purchase_no}？將從「${p.warehouse}」扣回本單進貨數量，紀錄仍會保留。`)) return
    setBusy(true)
    const r = await request('voidPurchase', { table: 'purchases', id: p.id })
    if (!r.error) { showToast('進貨單已作廢，指定倉庫庫存已沖回'); setDetail(null); await Promise.all([load(), reloadProducts()]) }
    setBusy(false)
  }

  return <>
    <div className="page-header">
      <div><h1 className="page-title">進貨單</h1><div className="page-sub">PURCHASES · {filtered.length} 筆</div></div>
      <div className="toolbar"><input className="form-control" style={{ width: 220 }} aria-label="搜尋進貨單" placeholder="單號、供應商、倉庫、品名…" value={search} onChange={e => setSearch(e.target.value)} /><label><input type="checkbox" checked={showVoided} onChange={e => setShowVoided(e.target.checked)} /> 顯示已作廢</label><button className="btn btn-primary" onClick={openNew} disabled={busy || !warehouses.length}>＋ 新增進貨單</button></div>
    </div>
    <div className="page-body">
      <p style={{ marginBottom: 16, color: 'var(--text2)' }}>進貨過帳後會直接增加所選倉庫的庫存，並同步更新商品總庫存。寄賣補貨若由總倉出貨，請改用「庫存管理 → 庫存調撥」。</p>
      {loadError && <p role="alert">{loadError} <button className="btn btn-ghost" onClick={load}>重試</button></p>}
      <div className="card table-wrap"><table><thead><tr>{['日期','進貨單號','供應商','倉庫','數量','未稅金額','稅額','含稅總額','狀態','操作'].map(x => <th key={x}>{x}</th>)}</tr></thead><tbody>
        {loading ? <tr><td colSpan={10}>載入中…</td></tr> : filtered.length === 0 ? <tr><td colSpan={10}><div className="empty-state">尚無符合的進貨單</div></td></tr> : filtered.map(p => <tr key={p.id}><td>{String(p.purchase_date).slice(0, 10)}</td><td>{p.purchase_no}</td><td>{p.supplier}</td><td>{p.warehouse}</td><td>{(p.items || []).reduce((n, i) => n + Number(i.qty), 0)}</td><td>{money(p.net_amount)}</td><td>{money(p.tax_amount)}</td><td>{money(p.total_amount)}</td><td><span className={`badge ${p.status === 'void' ? 'badge-red' : 'badge-green'}`}>{p.status === 'void' ? '已作廢' : '已入庫'}</span></td><td><button className="btn btn-ghost btn-sm" onClick={() => setDetail(p)}>查看</button>{p.status !== 'void' && <button className="btn btn-danger btn-sm" disabled={busy} onClick={() => voidDocument(p)}>作廢</button>}</td></tr>)}
      </tbody></table></div>
    </div>
    ...
    {open && <div className="modal-overlay"><div className="modal" style={{ width: 'min(1080px,96vw)', maxWidth: 1080 }}><div className="modal-header"><span className="modal-title">新增進貨單</span><button className="btn btn-ghost" disabled={busy} onClick={() => setOpen(false)}>關閉</button></div><div className="modal-body">
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 14 }}>
        ...
        <div className="form-group"><label className="form-label" htmlFor="purchase-warehouse">入庫倉庫 *</label><select id="purchase-warehouse" className="form-control" value={form.warehouse} disabled={busy} onChange={e => setForm(f => ({ ...f, warehouse: e.target.value }))}>{warehouses.map(w => <option key={w.id} value={w.name}>{w.name}</option>)}</select></div>
