...
export default function Orders({ showToast }) {
  ...
  const addToCart = variant => {
    ...
    const price = dealerPrice(selectedProduct.retail_price, selectedCustomer.discount, selectedTaxMode)
    setCartItems(prev => {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true)
    try {
      await addOrder({ customer_id: custId, total_amount: cartTotal, note: orderNote, items: cartItems, discount: +selectedCustomer.discount, sale_mode: selectedCustomer.sale_mode, tax_mode: selectedTaxMode })
      showToast('訂單已建立')
      setShowModal(false)
  ...
  return <>
    ...
    {showModal && <div className="modal-overlay" onClick={() => setShowModal(false)}><div className="modal modal-lg" onClick={e => e.stopPropagation()}><div className="modal-header"><span className="modal-title">開立新訂單</span><button className="btn btn-ghost btn-sm btn-icon" onClick={() => setShowModal(false)}>✕</button></div><div className="modal-body"><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
      ...
      <div>選擇商品</div><div style={{ position: 'relative', marginBottom: 12 }}><input className="form-control" placeholder="搜尋款式…" value={prodSearch} onFocus={() => setShowProdDrop(true)} onBlur={() => setTimeout(() => setShowProdDrop(false), 150)} onChange={e => setProdSearch(e.target.value)} />{showProdDrop && filteredProds.length > 0 && <DropDown>{filteredProds.map(p => <DropItem key={p.id} onMouseDown={() => { setSelectedProduct(p); setProdSearch(p.name); setShowProdDrop(false) }}>{p.name}<span style={{ color: 'var(--gold)', marginLeft: 8, fontSize: 11, fontFamily: 'var(--font-mono)' }}>{selectedCustomer?.discount != null ? `未稅 NT$ ${dealerPrice(p.retail_price, selectedCustomer.discount, selectedTaxMode)}` : `含稅零售價 ${p.retail_price}`}</span></DropItem>)}</DropDown>}</div>
      {selectedProduct && <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 14 }}><div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>{selectedProduct.name}</div>{(() => { const colors=[...new Set((selectedProduct.variants||[]).map(v=>v.color))]; const sizes=[...new Set((selectedProduct.variants||[]).map(v=>v.size))]; return colors.map(color => <div key={color} style={{ marginBottom: 10 }}><div style={{ display:'flex',alignItems:'center',gap:6,marginBottom:6,fontSize:12,color:'var(--text2)' }}><span style={{ width:10,height:10,borderRadius:'50%',background:COLOR_MAP[color]||'#888',border:'1px solid rgba(0,0,0,0.1)' }} />{color}</div><div style={{ display:'flex',gap:5,flexWrap:'wrap' }}>{sizes.map(size => { const v=(selectedProduct.variants||[]).find(vv=>vv.color===color&&vv.size===size); if(!v)return null; return <button key={size} disabled={+v.stock_qty===0||selectedCustomer?.discount==null} onClick={()=>addToCart(v)} style={{ display:'flex',flexDirection:'column',alignItems:'center',padding:'4px 9px',borderRadius:4,cursor:+v.stock_qty===0?'not-allowed':'pointer',border:'1px solid var(--border2)',background:'var(--bg2)',opacity:+v.stock_qty===0?0.3:1,transition:'var(--transition)' }} onMouseEnter={e=>{if(+v.stock_qty>0)e.currentTarget.style.borderColor='var(--gold)'}} onMouseLeave={e=>e.currentTarget.style.borderColor='var(--border2)'}><span style={{ fontSize:12,fontFamily:'var(--font-mono)',fontWeight:700,color:'var(--text)' }}>{size}</span><span style={{ fontSize:10,color:+v.stock_qty<=5?'var(--amber)':'var(--text3)' }}>{v.stock_qty}</span></button>})}</div></div>) })()}<div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 6 }}>含稅零售價 {selectedProduct.retail_price} 元 → 未稅成交單價 {selectedCustomer?.discount == null ? '—' : dealerPrice(selectedProduct.retail_price, selectedCustomer.discount, selectedTaxMode)} 元（{taxModeLabel(selectedTaxMode)}）</div></div>}
      </div>

      <div><div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'var(--font-mono)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.08em' }}>訂單明細</div>{cartItems.length === 0 ? <div style={{ padding:'40px 0',textAlign:'center',color:'var(--text3)',fontSize:13,border:'1px dashed var(--border2)',borderRadius:'var(--radius)' }}>點擊左側尺碼加入商品</div> : <>{cartItems.map(item => <div key={item.variant_id} style={{ display:'flex',alignItems:'center',gap:10,padding:'9px 0',borderBottom:'1px solid var(--border)' }}><span style={{ width:8,height:8,borderRadius:'50%',background:COLOR_MAP[item.color]||'#888',border:'1px solid rgba(0,0,0,0.1)',flexShrink:0 }} /><div style={{ flex:1,minWidth:0 }}><div style={{ fontSize:13,fontWeight:500 }}>{item.product_name}</div><div style={{ fontSize:11,color:'var(--text3)' }}>{item.color} · {item.size} · 未稅 NT$ {item.price}</div></div><div style={{ display:'flex',alignItems:'center',gap:5 }}><button onClick={()=>updateCartQty(item.variant_id,item.qty-1)} style={{ width:24,height:24,borderRadius:4,border:'1px solid var(--border2)',background:'var(--bg3)',cursor:'pointer',fontSize:14,display:'flex',alignItems:'center',justifyContent:'center' }}>−</button><span style={{ fontFamily:'var(--font-mono)',fontSize:14,minWidth:20,textAlign:'center' }}>{item.qty}</span><button onClick={()=>updateCartQty(item.variant_id,item.qty+1)} style={{ width:24,height:24,borderRadius:4,border:'1px solid var(--border2)',background:'var(--bg3)',cursor:'pointer',fontSize:14,display:'flex',alignItems:'center',justifyContent:'center' }}>＋</button></div><span style={{ fontFamily:'var(--font-mono)',fontSize:13,color:'var(--gold)',fontWeight:700,minWidth:54,textAlign:'right' }}>{(item.qty*item.price).toLocaleString()}</span><button onClick={()=>updateCartQty(item.variant_id,0)} style={{ background:'none',border:'none',color:'var(--text3)',cursor:'pointer',fontSize:14 }}>✕</button></div>)}<div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',paddingTop:12 }}><span style={{ fontSize:12,color:'var(--text3)' }}>{cartItems.reduce((s,i)=>s+i.qty,0)} 件</span><span style={{ fontFamily:'var(--font-display)',fontSize:22,color:'var(--gold)' }}>未稅 {cartNet.toLocaleString()}<br /><small>稅額 5%：{cartTax.toLocaleString()}（{selectedTaxMode === 'inclusive' ? '已內含' : '外加'}）</small><br />含稅合計 {cartTotal.toLocaleString()}</span></div></>}
    ...
    </div></div><div className="modal-footer"><button className="btn btn-ghost" onClick={()=>setShowModal(false)}>取消</button><button className="btn btn-primary" disabled={!custId||selectedCustomer?.discount==null||cartItems.length===0||saving} onClick={handleSubmit}>{saving ? <span className="spinner" style={{ width:14,height:14 }} /> : `建立${selectedCustomer?.sale_mode==='consignment'?'寄賣單（含稅貨值）':'銷貨單（含稅）'} NT$ ${cartTotal.toLocaleString()}`}</button></div></div></div>}
