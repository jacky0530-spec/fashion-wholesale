...
const fields = {
 ...
 customers: ['customer_name','name','shop_name','tax_id','line_nick','phone','contact_email','address','customer_type','sale_mode','discount','tax_mode','credit_limit','note'],
 orders: ['note'],
 ...
 try {
  ...
  if(action==='list') {
   const selects={
    ...
    consignment_settlements:`SELECT s.id,s.settlement_no,s.settlement_date,s.net_amount,s.tax_amount,s.total_amount,s.order_id,s.created_at,w.name AS warehouse,COALESCE(c.customer_name,c.name) AS customer_name,c.name AS contact_name,c.shop_name,COALESCE((SELECT json_agg(json_build_object('id',i.id,'variant_id',i.variant_id,'system_qty',i.system_qty,'counted_qty',i.counted_qty,'sold_qty',i.sold_qty,'unit_price',i.unit_price,'product_name',p.name,'product_code',p.product_code,'color',pv.color,'size',pv.size) ORDER BY p.name,pv.color,pv.size) FROM consignment_settlement_items i JOIN product_variants pv ON pv.id=i.variant_id JOIN products p ON p.id=pv.product_id WHERE i.settlement_id=s.id),'[]') AS items FROM consignment_settlements s JOIN warehouses w ON w.id=s.warehouse_id JOIN customers c ON c.id=s.customer_id ORDER BY s.settlement_date DESC,s.created_at DESC`,
    custom_orders:`SELECT o.*,json_build_object('name',c.name,'customer_name',c.customer_name,'shop_name',c.shop_name) AS customer,COALESCE((SELECT json_agg(i ORDER BY i.sort_order,i.id) FROM custom_order_items i WHERE i.custom_order_id=o.id),'[]') AS items,COALESCE((SELECT json_agg(p ORDER BY p.payment_date,p.created_at) FROM custom_order_payments p WHERE p.custom_order_id=o.id),'[]') AS payments FROM custom_orders o JOIN customers c ON c.id=o.customer_id ORDER BY o.order_date DESC,o.created_at DESC`,
    products:`SELECT p.*,
    ...
    orders:`SELECT o.*, json_build_object('name',c.name,'customer_name',c.customer_name,'shop_name',c.shop_name) AS customer, COALESCE((SELECT json_agg(i) FROM order_items i WHERE i.order_id=o.id),'[]') AS items FROM orders o JOIN customers c ON c.id=o.customer_id ORDER BY o.order_date DESC`,
    returns:`SELECT r.*, json_build_object('name',c.name,'customer_name',c.customer_name,'shop_name',c.shop_name) AS customer, json_build_object('order_date',o.order_date) AS "order" FROM returns r JOIN customers c ON c.id=r.customer_id LEFT JOIN orders o ON o.id=r.order_id ORDER BY r.created_at DESC`
